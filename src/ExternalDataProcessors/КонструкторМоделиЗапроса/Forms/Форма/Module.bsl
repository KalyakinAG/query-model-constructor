...
Процедура ДобавитьТекстПоля(Строки, ЗапросПакета, Выражение)
	...
	Если Выражение.СодержитАгрегатнуюФункцию() Тогда
		АгрегатнаяФункция = "";
		...
		ИначеЕсли СтрНачинаетсяС(Поле, "МИНИМУМ(") Тогда
			...
			Поле = Сред(Поле, ЧислоСимволов + 1, СтрДлина(Поле) - ЧислоСимволов - 1);
		Иначе
			ВызватьИсключение СтрШаблон("Не определена агрегатная функция %1", Выражение);
		КонецЕсли;
		Псевдоним = Псевдоним(Поле);
		Если Колонка.Поля.Количество() = 1 И Псевдоним = Колонка.Псевдоним Тогда
			Строки.Добавить(СтрШаблон("МодельЗапроса.%1(""%2"");", АгрегатнаяФункция, Поле));
			Возврат;
		КонецЕсли;
		Строки.Добавить(СтрШаблон("МодельЗапроса.%1(""%2"", ""%3"");", АгрегатнаяФункция, ОбщийКлиентСервер.ЭкранироватьТекст(Поле), Колонка.Псевдоним));
		Возврат;
	КонецЕсли;
	...
	Если СтрНачинаетсяС(Поле, "ЕСТЬNULL(") Тогда
		...
		Псевдоним = Псевдоним(Поле);
		Если Колонка.Поля.Количество() = 1 И Псевдоним = Колонка.Псевдоним Тогда
			Строки.Добавить(СтрШаблон("МодельЗапроса.Поле(""%1"", , %2);", Поле, ПолеЕстьNull));
			Возврат;
		КонецЕсли;
		Строки.Добавить(СтрШаблон("МодельЗапроса.Поле(""%1"", ""%2"", ""%3"");", ОбщийКлиентСервер.ЭкранироватьТекст(Поле), Колонка.Псевдоним, ОбщийКлиентСервер.ЭкранироватьТекст(ПолеЕстьNull)));
		Возврат;
	КонецЕсли;
