...
Процедура ДобавитьТекстОператора(Строки, ЗапросПакета, ОператорВыбрать, ЭтоПервыйОператор, Уровень = 0)
	...
	Если ЭтоПервыйОператор Тогда
		Если ЗапросПакета.ВыбиратьРазрешенные Тогда
			Строки.Добавить(СтрШаблон("МодельЗапроса.Выбрать(%1, Истина);", ВыбратьПервые));
		Иначе
			Строки.Добавить(СтрШаблон("МодельЗапроса.Выбрать(%1);", ВыбратьПервые));
		КонецЕсли;
	Иначе
		Строки.Добавить(СтрШаблон("МодельЗапроса.Объединить%1();", ?(ОператорВыбрать.ТипОбъединения = ТипОбъединенияСхемыЗапроса.ОбъединитьВсе, "Все", "")));
	КонецЕсли;
