...
&НаСервере
Функция ИзвлечьСтруктуруВыражения(Выражение)
	Если Прав(Выражение, 1) <> ")" Тогда
		Возврат Неопределено;
	КонецЕсли;
	ИмяФункции = ВРег(Лев(Выражение, СтрНайти(Выражение, "(") - 1));
	ВыражениеФункции = Выражение;
	ВыражениеФункции = Прав(ВыражениеФункции, СтрДлина(ВыражениеФункции) - СтрНайти(ВыражениеФункции, "("));
	ВыражениеФункции = Лев(ВыражениеФункции, СтрДлина(ВыражениеФункции) - 1);
	Если ИмяФункции = "КОЛИЧЕСТВО" Тогда
		Если СтрНачинаетсяС(ВРег(ВыражениеФункции), "РАЗЛИЧНЫЕ") Тогда
			ВыражениеФункции = Прав(ВыражениеФункции, СтрДлина(ВыражениеФункции) - СтрДлина("РАЗЛИЧНЫЕ "));
			Возврат Новый Структура("ИмяФункции, Выражение", "КоличествоРазличных", ВыражениеФункции);
		КонецЕсли;
		Возврат Новый Структура("ИмяФункции, Выражение", ИмяФункции, ВыражениеФункции);
	КонецЕсли;						 
	Если ИмяФункции = "АВТОНОМЕРЗАПИСИ" Тогда
		Возврат Новый Структура("ИмяФункции", "Автономер");
	КонецЕсли;						 
	Если СтрРазделить("СУММА,КОЛИЧЕСТВО,СРЕДНЕЕ,МАКСИМУМ,МИНИМУМ", ",").Найти(ИмяФункции) <> Неопределено Тогда
		Возврат Новый Структура("ИмяФункции, Выражение", ОбщийКлиентСервер.CamelCase(ИмяФункции), ВыражениеФункции);
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
...
Процедура ДобавитьТекстПоля(Строки, ЗапросПакета, Выражение)
	...
	СтруктураВыражения = ИзвлечьСтруктуруВыражения(Выражение);
	Если ЗначениеЗаполнено(СтруктураВыражения) Тогда
		Если СтруктураВыражения.Свойство("Выражение") Тогда
			Строки.Добавить(СтрШаблон("МодельЗапроса.%1(""%2"", ""%3"");", СтруктураВыражения.ИмяФункции, ОбщийКлиентСервер.ЭкранироватьТекст(СтруктураВыражения.Выражение), Колонка.Псевдоним));
			Возврат;
		КонецЕсли;
		Строки.Добавить(СтрШаблон("МодельЗапроса.%1(""%2"");", СтруктураВыражения.ИмяФункции, Колонка.Псевдоним));
		Возврат;
	КонецЕсли;
	Псевдоним = Псевдоним(Строка(Выражение));
	Если Колонка.Поля.Количество() = 1 И Псевдоним = Колонка.Псевдоним Тогда
		Строки.Добавить(СтрШаблон("МодельЗапроса.Поле(""%1"");", Выражение));
		Возврат;
	КонецЕсли;
	Строки.Добавить(СтрШаблон("МодельЗапроса.Поле(""%1"", ""%2"");", ОбщийКлиентСервер.ЭкранироватьТекст(Выражение), Колонка.Псевдоним));
КонецПроцедуры
